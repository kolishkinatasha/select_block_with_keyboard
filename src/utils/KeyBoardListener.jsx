...
const KeyBoardListener = () => {
  ...
  const getRowAndCategoryIndex = (dayIndex, currentTaskId, currentCategoryId, step) => {
    ...
    inputList.forEach((input, index) => {
      ...
      // если индекс задачи и категории элемента совпадает с текущим выбранным элементом
      ...
      if (taskId && currentTaskId === +taskId && categoryId && currentCategoryId === +categoryId) {
        ...
        // задаем новые значения для строки и категории
        if (newInput) {
  ...
  useEffect(() => {
    ...
    if (selectedDay && !isInputFocused) {
      ...
      // задаем новый выбранный элемент через сеттер
